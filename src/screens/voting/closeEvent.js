    ...
    {items.map((item, index)=>{
      ...
      return (
        <TouchableOpacity activeOpacity={0.8} onPress={()=>onPress(item, index)} style={{flexDirection:'row', height:34, marginTop:8, borderRadius:10, backgroundColor:commonColors.lightTheme, paddingHorizontal:10, alignItems:'center'}}>
            ...
            <Text style={{fontSize:13, color:'#333', flex:1, marginLeft:15}}>{item.sport} | {item.league} | {item.event_hostname} </Text>
...
export default class CloseEvent extends PureComponent{
    constructor(props){
        super(props)
        this.state={
          sport:'',
            ...
            <View style={styles.container}>
                <View style={{margin:30, flex:1}}>
                  <View style={{backgroundColor:commonColors.lightTheme, justifyContent:'center', alignItems:'center', height:34, borderTopLeftRadius:20, borderTopRightRadius:20}}>
                    <Text style={{fontSize:12, color:'red'}}>CLICK EVENT TO SEE VOTING RESULTS</Text>
                  </View>
                  <ScrollView style={{marginTop:8}} showsVerticalScrollIndicator={false}>
                    <ShowItemList items={this.props.closed_events} onPress={(item, index)=>{Actions.CloseResult({item})}}/>
                  </ScrollView>
                </View>
            </View>
        )
